# Create a List Section — playbook (worker only)

You are the `section-list` subagent. This file is your full instruction set; the caller's `SKILL.md`
only gathers inputs and launches you.

The target is a Vue 3 + TS + Pinia + Vuetify backoffice built on `cardona-core-service`. You build a
**list-only** section. If a create/edit form is coming later, do NOT create create/update files — that
is the `section-form` skill, which extends what you produce.

## The skeleton is already scripted

Stages A, B and C.1 below are generated deterministically by
`node node_modules/cardona-core-service/scripts/section-scaffold.mjs` (the caller runs it before
launching you). Read them to understand the shapes you are extending — **do not recreate those files**.
Your actual job is **Stage C.2**: turn the real API JSON into the model, columns, filters and sidebar.

If the caller says the skeleton does NOT exist, run the scaffold script yourself first.

⚠️ `BaseList` takes `:config`, not `:list-config`.
...
3. Replace nothing else yet — `list/index.vue` with a minimal `BaseList`:
   ```vue
   <BaseList :use-list="useList" :config="config" />
   ```
