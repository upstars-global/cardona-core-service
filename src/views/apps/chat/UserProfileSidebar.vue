<template>
  <div class="chat-profile-sidebar" :class="{ show: shallShowUserProfileSidebar }">
    <!-- Header -->
    <header class="chat-profile-header">
      <span class="close-icon">
        <feather-icon icon="XIcon" @click="$emit('close-sidebar')" />
      </span>

      <div class="header-profile-sidebar">
        <div class="avatar box-shadow-1 avatar-xl avatar-border">
          <b-avatar size="70" :src="profileUserData.avatar" />
          <span class="avatar-status-xl" :class="`avatar-status-${profileUserData.status}`" />
        </div>
    ...
    >
      ...
      <b-form-radio-group id="user-status-options" v-model="profileUserData.status" stacked>
      ...
      <ul v-if="profileUserData.settings" class="list-unstyled">
        <!-- Two Step Auth -->
        ...
        <li class="d-flex justify-content-between align-items-center mb-1">
          <div class="d-flex align-items-center">
            <feather-icon icon="BellIcon" size="18" class="mr-75" />
            <span class="align-middle">Notification</span>
          </div>
          <b-form-checkbox v-model="profileUserData.settings.isNotificationsOn" switch />
        </li>

        <!-- Invite Friends -->
        <li class="mb-1 d-flex align-items-center cursor-pointer">
          <feather-icon icon="UserIcon" class="mr-75" size="18" />
          <span class="align-middle">Invite Friends</span>
        </li>

        <!-- Delete Account -->
        <li class="d-flex align-items-center cursor-pointer">
          <feather-icon icon="TrashIcon" class="mr-75" size="18" />
