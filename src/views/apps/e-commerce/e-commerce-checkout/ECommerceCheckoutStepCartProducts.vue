    ...
    <b-card v-for="product in products" :key="product.id" class="ecommerce-card" no-body>
      ...
      <b-card-body>
        ...
        <div class="item-quantity">
          ...
          <b-form-spinbutton v-model="product.qty" size="sm" class="ml-75" inline />
        </div>
        <span class="delivery-date text-muted"
          >Delivery by
          {{
            formatDate(product.shippingDate, { month: 'short', day: 'numeric', weekday: 'short' })
          }}</span
        >
        <span class="text-success"
          >{{ product.discountPercentage }}% off {{ product.offers }} offers Available</span
        >
      </b-card-body>
