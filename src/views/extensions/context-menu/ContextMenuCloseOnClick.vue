<template>
  <b-card-code title="Close On Click">
    <b-card-text>
      ...
      <code>false</code>
      <span>
        , the context menu will not automatically close when clicked on. You can toggle this feature
        on and off in this demo by using the checkbox below.</span
      >
    </b-card-text>
    ...
    <!-- context -->
    <vue-context ref="menuProp" :close-on-click="false">
      <li v-for="data in menuData" :key="data.icon">
        <b-link class="d-flex align-items-center" @click="optionClicked(data.text, data.icon)">
