...
export const filterDemoList = (list, filter) => {
    const { search, createdFrom, createdTo, tagsIds, type, gameId, isActive } = filter;
    let result = list;
    if (search) {
        const searchString = search?.toLowerCase();
        result = result.filter((item) => item.name.toLowerCase().includes(searchString) ||
            item.id.toLowerCase().includes(searchString) ||
            item.comment.toLowerCase().includes(searchString));
    }
    if (!!createdFrom && !!createdTo) {
        result = result.filter((item) => {
            return (moment(item.newDate).isSameOrAfter(createdFrom) &&
...
//# sourceMappingURL=filterDemoList.js.map